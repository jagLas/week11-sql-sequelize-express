...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    for (let i = 0; i < musicianInstruments.length; i++) {
      ...
      const editMusician = await Musician.findOne({where: musician})
      const instrumentIds = await Instrument.findAll({
        where: {
          [Op.or]: instruments
        }
      });
      await editMusician.addInstruments(instrumentIds);
    }
  },
  ...
  down: async (queryInterface, Sequelize) => {
    for (let i = 0; i < musicianInstruments.length; i++) {
      ...
      await editMusician.removeInstruments(instrumentIds);
    }
  }
};
